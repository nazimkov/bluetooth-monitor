...
public sealed class DevicesFlowTests(AppLifecycle app) : E2ETestBase(app)
{
    ...
    {
        CaptureOnFailure(nameof(SelectDevice_UpdatesHero), () =>
        {
            Shell.GoToDevices();
            ...
            UiWait.WaitUntil(
                () => Devices.HeroDeviceName().Contains("E2E Earbuds", StringComparison.OrdinalIgnoreCase),
                TimeSpan.FromSeconds(10),
                $"Hero did not show E2E Earbuds (was '{Devices.HeroDeviceName()}').");

            Devices.SelectDevice("e2e-headset");
            UiWait.WaitUntil(
                () => Devices.HeroDeviceName().Contains("E2E Headset", StringComparison.OrdinalIgnoreCase),
                TimeSpan.FromSeconds(10),
    ...
    {
        CaptureOnFailure(nameof(Rescan_KeepsFakeDevices), () =>
        {
            Shell.GoToDevices();
            Devices.WaitUntilLoaded();
            Devices.Rescan();
