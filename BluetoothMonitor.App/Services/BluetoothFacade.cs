...
public sealed class BluetoothFacade : IBluetoothFacade
{
    ...
    public async Task<string?> FindDeviceIdAsync(string deviceName)
    {
        ...
        if (!string.IsNullOrWhiteSpace(leId))
            return leId;
    ...
    {
        if (
            info.Properties.TryGetValue("System.Devices.Aep.Bluetooth.Cod.Major", out var majorObj)
        ...
        if (name.Contains("buds") || name.Contains("earbud") || name.Contains("airpod"))
            return DeviceKind.Earbuds;
        if (name.Contains("speaker") || name.Contains("boom"))
            return DeviceKind.Speaker;
        if (name.Contains("head") || name.Contains("phone"))
            return DeviceKind.OverEar;
    ...
    {
        if (
            info.Properties.TryGetValue("System.Devices.Aep.IsConnected", out var value)
            && value is bool b
        )
        {
            return b;
