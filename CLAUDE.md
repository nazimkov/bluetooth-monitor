...
FlaUI + xUnit UI tests live in `BluetoothMonitor.E2E`. They launch the built `BluetoothMonitor.exe` with `BLUETOOTHMONITOR_E2E=1` (fake Bluetooth, no-op tray/toasts/startup, isolated settings dir). Requires an interactive Windows desktop session (not headless Server Core).
...
- **`BluetoothMonitor.App`** — WinUI 3 packaged app (MSIX, `WindowsPackageType=MSIX`, `Platforms=x64`). Built around the "Bluetooth Monitor" Claude-Design mockup. Structure:
  ...
  - `Services/JsonSettingsService` persists to `%LOCALAPPDATA%\BluetoothMonitor\settings.json` with a 500ms debounce.
  ...
  - `Services/StartupTaskService` calls `Windows.ApplicationModel.StartupTask.GetAsync("BluetoothMonitorStartup")` — that StartupTask is declared in `Package.appxmanifest`. Relies on MSIX identity.
  ...
  - When `BLUETOOTHMONITOR_E2E=1`, DI swaps in `FakeBluetoothFacade` + no-op tray/startup/notifications and settings write under `BLUETOOTHMONITOR_E2E_SETTINGS_DIR`.
...
- Mockup's CSS vars → `Styles/Colors.xaml` with Light/Dark `ThemeDictionaries`. Consumers bind `{ThemeResource BluetoothMonitorAccentBrush}` etc.
